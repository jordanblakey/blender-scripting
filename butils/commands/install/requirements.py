...
def install_requirements():
    ...
    for package_line in requirements:
        ...
        if not package_name_full or package_name_full.startswith("#"): # Skip empty lines or comments
            continue
        
        # Get clean package name for path and skip check
        package_name = package_name_full.split("==")[0].split("~=")[0].split(">=")[0].split("<=")[0].split("!=")[0].strip()

        if package_name in ['fake-bpy-module-latest', 'blender-stubs']:
            print(f"Skipping installation of {package_name} into Blender's path (for IDE/external use only).")
            continue
        ...
        cmd = [
            sys.executable, 
            "-m", "pip", "install", package_name_full, # Use full spec here
            "--target", install_path,
            "--upgrade"
        ]
        
        print(f"Attempting to install {package_name_full} to {install_path} with command: {' '.join(cmd)}")
        try:
            ...
            if result.returncode == 0:
                ...
                if result.stdout: # Print stdout even on failure, it might contain useful info
                    print(f"Stdout:\n{result.stdout}")
                if result.stderr:
                    print(f"Stderr:\n{result.stderr}")
        except FileNotFoundError: 
            ...
            print(f"An error occurred while trying to pip install {package_name_full}: {e}", file=sys.stderr)